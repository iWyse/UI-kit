...
(() => {
    ...
    document.querySelectorAll(".form-group__quantity").forEach((wrapper => {
        ...
        const plusButton = wrapper.querySelector(".quantity-plus");
        const counterElement = wrapper.querySelector(".counter");
        if (!minusButton || !plusButton || !counterElement) return;
        let counter = Number(counterElement.textContent.trim()) || 0;
        minusButton.addEventListener("click", (event => {
            event.preventDefault();
    ...
    const closeAlert = document.getElementsByClassName("close-alert");
    for (let i = 0; i < closeAlert.length; i++) closeAlert[i].onclick = function() {
        let alert = this.parentElement;
        alert.style.opacity = "0";
        setTimeout((() => {
            alert.style.display = "none";
    ...
    const closeModal = document.getElementsByClassName("close-modal");
    for (let i = 0; i < closeModal.length; i++) closeModal[i].onclick = function() {
        let modal = this.parentElement;
        modal.style.opacity = "0";
        setTimeout((() => {
            modal.style.display = "none";
    ...
    window.addEventListener("scroll", (() => {
        let scrolled = document.documentElement.scrollTop;
        if (scrolled > 100 && !(scrolled > scrollPrev)) navigation.classList.add("navigation--fixed"); else navigation.classList.remove("navigation--fixed");
        scrollPrev = scrolled;
    }));
    window["FLS"] = true;
    isWebp();
